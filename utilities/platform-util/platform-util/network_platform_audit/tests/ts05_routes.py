...
def _resolve_route_kernel_if(ifname, ifaces):
    kernel_if = None
    if ifname:
...
def _find_kernel_route(kernel_routes_raw, cidr, network, gateway, metric, kernel_if, prefix):
    try:
    ...
    return route_found, matched_line, mismatch_details


def _check_route_src(hostname, cidr, network, matched_line, src_expected, kernel_if, ifname, mismatch_details):
...
def _log_route_result(cat, hostname, cidr, gateway, metric, kernel_if, ifname, mismatch_details):
...
def _collect_db_networks(db_routes):
    db_nets = set()
...
def _warn_kernel_extra_routes(cat, hostname, kernel_routes_raw, db_nets):
    for route_line in kernel_routes_raw.splitlines():
...
def test_routes_vs_kernel():
    ...
    sw_ver = _get_sw_version(local_hostname())
    if sw_ver is None:
        log("  [WARN] could not read sw_version - src kernel-route check will be skipped")
        src_expected = None
    else:
        src_expected = sw_ver >= (26, 3, 0)
        log(f"  sw_version={sw_ver[0]:02d}.{sw_ver[1]:02d}.{sw_ver[2]:03d}  "
            f"src_in_kernel_route={'expected' if src_expected else 'not expected'}")

    for hostname in get_host_names():
        log(f"[HOST] {hostname}")

        rc, out, _ = _run_system_list(["system", "host-route-list", hostname])
        ...
        for route in db_routes:
            ...
            kernel_if = _resolve_route_kernel_if(ifname, ifaces)

            route_found, matched_line, mismatch_details = _find_kernel_route(
                kernel_routes_raw, cidr, network, gateway, metric, kernel_if, prefix)
            ...
            _check_route_src(hostname, cidr, network, matched_line, src_expected,
                             kernel_if, ifname, mismatch_details)

            _log_route_result(cat, hostname, cidr, gateway, metric, kernel_if, ifname, mismatch_details)

        db_nets = _collect_db_networks(db_routes)

        _warn_kernel_extra_routes(cat, hostname, kernel_routes_raw, db_nets)
