...
def _get_clusterip(cat):
    ...
    if rc != 0 or not clusterip_out:
        ...
        return None
    return clusterip_out.strip().strip("'")


def _check_clusterip_connectivity(cat, pid, k8s_clusterip):
...
def _check_endpoint_connectivity(cat, pid, ep_ip, ep_port):
...
def test_cluster_nat():
    ...
    k8s_clusterip = _get_clusterip(cat)
    if not k8s_clusterip:
        return
    ...
    _check_clusterip_connectivity(cat, pid, k8s_clusterip)
    ...
    for ep_ip, ep_port in endpoints:
        _check_endpoint_connectivity(cat, pid, ep_ip, ep_port)
