...
def _check_networks_have_pools(cat, networks, pools_per_net):
    for net in networks:
...
def _check_floating_addrs_local(cat, networks, pools_per_net, pool_by_name, floating_nets):
    for net in networks:
        net_name = net.get("name", "?")
...
def _check_pool_gateways(cat, addrpools):
...
def _build_pool_ranges(addrpools):
    pool_ranges = []
    for pool in addrpools:
        pname = pool.get("name", "?")
...
def _check_floating_addrs_remote_host(cat, rhost, networks, pools_per_net, pool_by_name, floating_nets):
    floatings_checked = 0
    for net in networks:
        net_name = net.get("name", "?")
        if net_name not in floating_nets:
...
def _populate_multicast_subnets(addrpools):
...
def test_addrpools():
    ...
    pool_net_type = {
        p.get("name", ""): net_type
        for net_type, pools in pools_by_type.items()
        for p in pools
    }
    ...
    _check_networks_have_pools(cat, networks, pools_per_net)

    floating_nets = {"mgmt", "oam", "cluster-host"}
    _check_floating_addrs_local(cat, networks, pools_per_net, pool_by_name, floating_nets)

    _check_pool_gateways(cat, addrpools)

    pool_ranges = _build_pool_ranges(addrpools)
    _check_pool_overlaps(cat, pool_ranges, pool_net_type)
    ...
    for rhost in remote_hosts:
        ...
        _check_floating_addrs_remote_host(cat, rhost, networks, pools_per_net, pool_by_name, floating_nets)

    # Populate multicast subnets for test_heartbeat_extended()
    _populate_multicast_subnets(addrpools)
