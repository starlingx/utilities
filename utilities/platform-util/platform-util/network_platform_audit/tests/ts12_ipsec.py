...
def _check_ipsec_port_listening(cat):
...
def _build_pxeboot_ip_map(addn_hosts_path):
    pxeboot_ip_to_name = {}
    ...
    return pxeboot_ip_to_name


def _resolve_pxeboot_hostname(host_sysinv_name, pxeboot_ip_to_name):
    rc2, addr_out, _ = _run_system_list(["system", "host-addr-list", host_sysinv_name], runner=run_silent)
    if not addr_out:
...
def _check_controller_neighbor(cat, host, pxeboot_ip_to_name, addn_hosts_path):
    hostname = host.get("hostname", "?")

    pxeboot_h, _ = _resolve_pxeboot_hostname(hostname, pxeboot_ip_to_name)
    if not pxeboot_h:
        host_id = host.get("id", "")
        if not host_id:
...
def _check_system_nodes_conn(cat):
...
def _check_sa_count(cat, sas_out, expected_peer_count):
...
def _check_local_sa_ips(cat, sas_out):
...
def test_ipsec():
    ...
    _check_ipsec_port_listening(cat)
    ...
    pxeboot_ip_to_name = _build_pxeboot_ip_map(addn_hosts_path)

    for host in controller_neighbors:
        _check_controller_neighbor(cat, host, pxeboot_ip_to_name, addn_hosts_path)

    _check_system_nodes_conn(cat)

    rc, sas_out, _ = run_log_only("swanctl --list-sas")
    expected_peer_count = len(neighbors)
    _check_sa_count(cat, sas_out, expected_peer_count)

    _check_local_sa_ips(cat, sas_out)
