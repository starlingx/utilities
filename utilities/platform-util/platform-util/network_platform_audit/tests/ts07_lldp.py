...
def _parse_lldpctl_blocks(lldpctl_out):
    lldpctl_blocks = {}
    current_if = None
    current_lines = []
    ...
    if current_if:
        lldpctl_blocks[current_if] = "\n".join(current_lines)
    return lldpctl_blocks


def _check_chassis_mac(cat, hostname, sysinv_if, chassis, block):
...
def _check_local_port_mac(cat, hostname, sysinv_if, port_id, kernel_ifname):
    prefix = f"  {hostname}/{sysinv_if}"
...
def _check_neighbor(cat, hostname, neighbor, ifaces_list, lldpctl_blocks, lldpctl_out):
    ...
    if chassis:
        _check_chassis_mac(cat, hostname, sysinv_if, chassis, block)

    _check_local_port_mac(cat, hostname, sysinv_if, port_id, kernel_ifname)
...
def test_lldp():
    ...
    if not tool_available("lldpctl"):
        log_result("lldpctl not available - LLDP daemon not running", "FAILED")
        state.category_failures[cat].append("lldpctl not available - LLDP daemon not running")
        return

    for hostname in get_host_names():
        log(f"[HOST] {hostname}")

        rc, out, _ = _run_system_list(["system", "host-lldp-neighbor-list", hostname])
        ...
        lldpctl_blocks = _parse_lldpctl_blocks(lldpctl_out)

        ifaces_list = _get_if_list(hostname)

        for neighbor in db_neighbors:
            _check_neighbor(cat, hostname, neighbor, ifaces_list, lldpctl_blocks, lldpctl_out)
