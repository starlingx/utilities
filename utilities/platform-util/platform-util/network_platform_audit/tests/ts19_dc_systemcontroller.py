...
def _extract_subcloud_fields(props):
    ...
    return (availability, deploy_status, gateway_ip, mgmt_gateway_ip,
            mgmt_start_ip, mgmt_subnet, sc_gw_ip)


def _check_availability(cat, sc_name, availability):
...
def _check_deploy_status(cat, sc_name, deploy_status):
...
def _ping_check(cat, label, ip, fail_msg):
    flag = "-6" if ":" in ip else ""
    rc, _, _ = run_log_only(["ping"] + ([flag] if flag else []) + ["-c", "2", "-W", "2", ip])
    if rc == 0:
        log_result(f"  {label} reachable", "PASS")
    else:
        log_result(f"  {label} reachable", "FAILED")
        state.category_failures[cat].append(fail_msg)


def _check_sc_gateway(cat, sc_name, sc_gw_ip):
    if sc_gw_ip:
        _ping_check(
            cat,
            f"{sc_name}: systemcontroller gateway {sc_gw_ip}",
            sc_gw_ip,
            f"subcloud {sc_name}: systemcontroller gateway {sc_gw_ip} unreachable",
        )


def _check_oam_gateway(cat, sc_name, gateway_ip):
    if gateway_ip:
        _ping_check(
            cat,
            f"{sc_name}: OAM gateway {gateway_ip}",
            gateway_ip,
            f"subcloud {sc_name}: OAM gateway {gateway_ip} unreachable",
        )


def _check_mgmt_gateway(cat, sc_name, mgmt_gateway_ip):
    if mgmt_gateway_ip:
        _ping_check(
            cat,
            f"{sc_name}: mgmt gateway {mgmt_gateway_ip}",
            mgmt_gateway_ip,
            f"subcloud {sc_name}: mgmt gateway {mgmt_gateway_ip} unreachable",
        )


def _check_mgmt_start_ip_reachable(cat, sc_name, mgmt_start_ip):
    if mgmt_start_ip:
        _ping_check(
            cat,
            f"{sc_name}: mgmt start IP {mgmt_start_ip}",
            mgmt_start_ip,
            f"subcloud {sc_name}: mgmt start IP {mgmt_start_ip} unreachable",
        )


def _check_dns_resolution(cat, sc_name, mgmt_start_ip):
...
def _check_subnet_in_gnp(cat, sc_name, mgmt_subnet, gnps):
    if not mgmt_subnet:
        return
    found_in_gnp = any(
...
def _check_kernel_route(cat, sc_name, mgmt_subnet):
    if not mgmt_subnet:
        return
...
def _determine_ssh_target(sc_name, mgmt_start_ip):
    ...
    return ssh_target, ssh_target_lbl


def _check_remote_host_list(cat, sc_name, ssh_target, ssh_target_lbl):
    rc_sys, sys_out, _ = remote_run(
        ssh_target,
        "bash -c 'source /etc/platform/openrc && system host-list --nowrap'",
        use_sudo=False,
    )
    if rc_sys != 0:
        rc_sys, sys_out, _ = remote_run(
...
def _check_remote_default_route(cat, sc_name, ssh_target, ssh_target_lbl):
...
def _check_remote_k8s_nodes(cat, sc_name, ssh_target, ssh_target_lbl):
...
def _run_remote_subcloud_checks(cat, sc_name, ssh_target, ssh_target_lbl):
    ...
    _check_remote_host_list(cat, sc_name, ssh_target, ssh_target_lbl)
    _check_remote_default_route(cat, sc_name, ssh_target, ssh_target_lbl)
    _check_remote_k8s_nodes(cat, sc_name, ssh_target, ssh_target_lbl)


def _process_subcloud(cat, sc_name, gnps):
    log(f"\n[SUBCLOUD] {sc_name}")
    props = _dcmanager_subcloud_show(sc_name)

    (availability, deploy_status, gateway_ip, mgmt_gateway_ip,
     mgmt_start_ip, mgmt_subnet, sc_gw_ip) = _extract_subcloud_fields(props)

    _check_availability(cat, sc_name, availability)
    _check_deploy_status(cat, sc_name, deploy_status)
    _check_sc_gateway(cat, sc_name, sc_gw_ip)
    _check_oam_gateway(cat, sc_name, gateway_ip)
    _check_mgmt_gateway(cat, sc_name, mgmt_gateway_ip)
    _check_mgmt_start_ip_reachable(cat, sc_name, mgmt_start_ip)
...
def test_dc_systemcontroller():
    cat = "TestSuite 19 - DC / System Controller"
    desc = [
    ...
    if not tool_available("dcmanager"):
        log_result("dcmanager not available on systemcontroller", "FAILED")
        state.category_failures[cat].append("dcmanager not available on systemcontroller")
        return
    ...
    for sc_name in subclouds:
        _process_subcloud(cat, sc_name, gnps)
